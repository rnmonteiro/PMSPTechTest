...
namespace PMESP.TechTest.Dal.extensions
{
    ...
    public sealed class DynamicQuery
    {
        ...
        /// </returns>
        public static string GetInsertQuery(string tableName, dynamic item, bool identity = false)
        {
            PropertyInfo[] props = item.GetType().GetProperties();
            List<string> columns = new List<string>();

            if (!identity)
                columns = props.Select(p => p.Name).Where(s => s != "Id").ToList();
            else
                columns = props.Select(p => p.Name).ToList();


            return $"INSERT INTO {tableName} ({string.Join(",", columns)}) OUTPUT inserted.Id VALUES (@{string.Join(",@", columns)})";
        }
        ...
        {
            PropertyInfo[] props = item.GetType().GetProperties();
            string[] columns = props.Where(a => a.Name != "Id").Select(p => p.Name).ToArray();
                                     ...
                                     ref List<QueryParameter> queryProperties)
        {
            if (body.NodeType != ExpressionType.AndAlso && body.NodeType != ExpressionType.OrElse)
            {
                ...
                var propVal = Expression.Lambda(propertyValue).Compile().DynamicInvoke();

                queryProperties.Add(new QueryParameter(link, propertyName, propVal, opr));
            }
            else
            {
